...
export async function getBullhornConnectionSecrets() {
  const rows = await supabaseRest('integration_connections', {
    query: {
      ...
      select:
        'provider,status,token_ciphertext,refresh_token_ciphertext,access_token_expires_at,rest_url,metadata',
      limit: '1',
    },
  });
...
export async function updateBullhornTokens({ accessToken, refreshToken, expiresAt }) {
  const rows = await supabaseRest('integration_connections', {
    method: 'PATCH',
    query: { provider: 'eq.bullhorn', select: 'provider,status,access_token_expires_at,updated_at' },
    body: {
      status: 'connected',
