...
function storedSession(connection, diagnostics = {}) {
  return {
    BhRestToken: connection.restToken,
    restUrl: connection.rest_url,
    diagnostics: {
      accessTokenPresent: Boolean(connection.accessToken),
      refreshTokenPresent: Boolean(connection.refreshToken),
      accessTokenRefreshed: false,
      restSessionCreated: false,
      restSessionReused: true,
      ...diagnostics,
    },
  };
}

export async function getBullhornRestSession({ forceNew = false } = {}) {
  let connection;
  try {
  ...
  if (!forceNew && connection.restToken && connection.rest_url) {
    return storedSession(connection);
  }
  ...
  try {
    ...
    try {
      await updateBullhornRestSession({
        restToken: session.BhRestToken,
        restUrl: session.restUrl,
      });
    } catch (error) {
      throw sessionError('session_store', 'Bullhorn REST session could not be stored.', error);
    }
    ...
    return {
      ...
      diagnostics: {
        ...
        restSessionReused: false,
      },
    };
  } catch (error) {
    if (error instanceof BullhornSessionError) throw error;

    // A parallel request may have created and stored the replacement session
    // while this Bullhorn login was in flight. Reuse it instead of failing.
    try {
      const latest = await getBullhornConnectionSecrets();
      const anotherLoginWon =
        latest?.restToken &&
        latest?.rest_url &&
        latest.restTokenCiphertext !== connection.restTokenCiphertext;
      if (anotherLoginWon) {
        return storedSession(latest, { accessTokenRefreshed });
      }
    } catch {
      // Preserve the original Bullhorn login failure below.
    }
