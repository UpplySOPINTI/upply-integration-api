...
// Bullhorn does not support JPQL /query for Candidate or Note, so keep their
// Lucene /search adapter explicit.
const BULLHORN_SEARCH_ENTITIES = new Set(['Candidate', 'Note']);
const BULLHORN_SEARCH_SORT_OVERRIDES = Object.freeze({ Note: null });
// ClientCorporation supports /query in this tenant, but not the isDeleted field.
// Use an always-positive id filter so company records remain discoverable.
const BULLHORN_QUERY_FILTER_OVERRIDES = Object.freeze({ ClientCorporation: 'id>0' });
...
export async function queryBullhornEntityPage({
  ...
  const requestPage = (currentSession) => {
    ...
    if (operation === 'search') {
      ...
      const searchSort = Object.hasOwn(BULLHORN_SEARCH_SORT_OVERRIDES, entity)
        ? BULLHORN_SEARCH_SORT_OVERRIDES[entity]
        : orderBy;
      if (searchSort) endpoint.searchParams.set('sort', searchSort);
    } else {
      const queryWhere =
        where === DEFAULT_ACTIVE_FILTER && Object.hasOwn(BULLHORN_QUERY_FILTER_OVERRIDES, entity)
          ? BULLHORN_QUERY_FILTER_OVERRIDES[entity]
          : where;
      endpoint.searchParams.set('where', queryWhere);
  ...
  if (response.status === 401) {
    const recreatedSession = await getBullhornRestSession({ forceNew: true });
